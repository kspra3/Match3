// Match 3 Game with JavaScript, HTML and CSS.
...
function createBoard() {
  for (let i = 0; i < width*width; i++) {
    ...
    let randomColor = Math.floor(Math.random() * tokens.length)
    square.style.backgroundImage = tokens[randomColor]
...
function dragEnd() {
    ...
    // Making sure tokens being swapped are in adjacent row or column.
    ...
    for (i = 0; i < 55; i ++) {
        if(squares[i + width].style.backgroundImage === '') {
            ...
            if (isFirstRow && (squares[i].style.backgroundImage === '')) {
              let randomColor = Math.floor(Math.random() * tokens.length)
              squares[i].style.backgroundImage = tokens[randomColor]
            }
        }
    }
}
